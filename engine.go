...
type (
	engineConfig struct {
		Pool    int    `toml:"pool"`
		File    string `toml:"file"` //自定义pool的toml文件
...
// Configure
func (module *engineModule) Configure(config Map) {
	// if config.Engine.Pool <= 0 {
	// 	//不做处理，0表示不限制，
	// 	config.Engine.Pool = 0
	// }
...
func (module *engineModule) Initialize() {
	...
	// err := loading(path.Join(module.config.File), pools)
	// if err == nil {
	// 	for key, pool := range pools {
	// 		if method, ok := module.methods[key]; ok {
	// 			method.pool = pool
	// 			module.methods[key] = method
	// 		}
	// 	}
	// }
}
...
func (module *engineModule) Service(name string, config Service, override bool) {
	...
	method := Method{group, 0, 0, config.Name, config.Desc, config.Alias, config.Nullable, config.Args, config.Data, config.Setting, config.Coding, config.Action, config.Token, config.Auth}
...
func (module *engineModule) call(ctx *context, name string, value Map, settings ...Map) (Map, Res, string) {
	...
	case func(*Process):
		ff(process)
	case func(*Process) Res:
		result = ff(process)
		//查询是否
	case func(*Process) bool:
		ok := ff(process)
		if ok {
			result = OK
		} else {
		...
		//查询单个
	case func(*Process) Map:
		data = ff(process)
	case func(*Process) (Map, Res):
		data, result = ff(process)
	...
	case func(*Process) []Map:
		...
		tttt = engineInvokes
	case func(*Process) ([]Map, Res):
		items, res := ff(process)
		data = Map{"items": items}
		result = res
		tttt = engineInvokes

		//统计的玩法
	case func(*Process) int:
		count := ff(process)
		data = Map{"count": float64(count)}
		tttt = engineInvokee
	case func(*Process) int64:
		count := ff(process)
		data = Map{"count": float64(count)}
		tttt = engineInvokee
	case func(*Process) float64:
		count := ff(process)
		data = Map{"count": count}
		tttt = engineInvokee

		//查询分页的玩法
	case func(*Process) ([]Map, int64):
		items, count := ff(process)
		data = Map{"count": count, "items": items}
		tttt = engineInvoking
	case func(*Process) ([]Map, int64, Res):
		items, count, res := ff(process)
		result = res
		data = Map{"count": count, "items": items}
		tttt = engineInvoking
	case func(*Process) (int64, []Map):
		count, items := ff(process)
		data = Map{"count": count, "items": items}
		tttt = engineInvoking
	case func(*Process) (int64, []Map, Res):
		count, items, res := ff(process)
		result = res
		data = Map{"count": count, "items": items}
		tttt = engineInvoking

	case func(*Process) (Map, []Map):
		item, items := ff(process)
		data = Map{"item": item, "items": items}
		tttt = engineInvoker
	case func(*Process) (Map, []Map, Res):
		item, items, res := ff(process)
		result = res
		data = Map{"item": item, "items": items}
		tttt = engineInvoker
	}
...
func (process *Process) Base(bases ...string) DataBase {
...
func invokingDataConfig(childrens ...Vars) Vars {
	...
	config := Vars{
		"count": Var{
			Type: "int", Required: true, Default: 0, Name: "统计数", Desc: "统计数",
		},
		"items": Var{
			Type: "[json]", Required: true, Name: "数据列表", Desc: "数据列表",
			Children: children,
		},
	}
	return config
...
func invokeeDataConfig() Vars {
	config := Vars{
		"count": Var{
			Type: "float", Required: true, Default: 0, Name: "统计数", Desc: "统计数",
		},
	}
	return config
}
...
func invokerDataConfig() Vars {
	config := Vars{
		"item": Var{
			Type: "json", Required: true, Name: "数据", Desc: "数据",
			// Children: children,
		},
		"items": Var{
			Type: "[json]", Required: true, Name: "数据列表", Desc: "数据列表",
